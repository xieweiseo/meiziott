...
function UpFtp($callBack = "image", $width = 100, $height = 100, $image = "")
{

    echo '<iframe scrolling="no" frameborder="0" border="0" onload="this.height=this.contentWindow.document.body.scrollHeight;this.width=this.contentWindow.document.body.scrollWidth;" width=' . $width . ' height="' . $height . '"  src="' . U('Upload/uploadftp',
            array('Width' => $width, 'Height' => $height, 'BackCall' => $callBack)) . '"></iframe>
         <input type="hidden" ' . 'value = "' . $image . '"' . 'name="' . $callBack . '" id="' . $callBack . '">';
}
...
function UpFile( $width = 100, $height = 100, $callBack1 = "", $callBack2 = "",$callBack3 = "",$callValue1 = "",$callValue2 = "",$callValue3 = "")
{

    echo '<iframe scrolling="no" frameborder="0" border="0" onload="this.height=this.contentWindow.document.body.scrollHeight;this.width=this.contentWindow.document.body.scrollWidth;" width=' . $width . ' height="' . $height . '"  src="' . U('Upload/uploadfile',
        array('Width' => $width, 'Height' => $height, 'BackCall' => 'video')) . '"></iframe>
         <input type="hidden" ' . 'value = "' . $callValue1 . '"' . 'name="' . $callBack1 . '" id="' . $callBack1 . '">
         <input type="hidden" ' . 'value = "' . $callValue2 . '"' . 'name="' . $callBack2 . '" id="' . $callBack2 . '">    
         <input type="hidden" ' . 'value = "' . $callValue3 . '"' . 'name="' . $callBack3 . '" id="' . $callBack3 . '">'             
    ;
}
...
{
    //获取当前sid下的所有子栏目的id
    $categorys = M("material_type")->where("material_type_pid = {$sid}")->select();

    $array = array_merge($array, array($sid));
    foreach ($categorys as $category) {
        category_get_sons($category['aterial_type_id'], $array);
    }
    $data = $array;
    unset($array);
    return $data;
