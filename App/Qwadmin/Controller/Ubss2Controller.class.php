...
class Ubss2Controller extends  ComController
{
    //上报
    ...
    public  function statistics(){
        ...
        //dump($device_map_county);
        //dump($device_county_active);exit;
        ...
        //dump($area_county);exit;
        //dump($device_info_county);
        //dump($device_info);
    ...
    private function get_area_sort($map_info, $sort_arr=''){         
        ...
        if(!empty($map_info) && !empty($sort_arr) && is_array($map_info) && is_array($sort_arr)){
    ...
    private function get_iom_area($iom_id = ''){
        ...
        if(!empty($iom_id)){
            $iom_value = !empty($iom_code[$iom_id])?$iom_code[$iom_id]:'';  
