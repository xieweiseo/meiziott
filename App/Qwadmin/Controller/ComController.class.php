...
class ComController extends  BaseController
{
    ...
    public function _initialize()
    {
        ...
        //$ss = $Auth->check(CONTROLLER_NAME . '/' . ACTION_NAME,$UID);              
    ...
    protected function getCategoryMenu($items, $id = 'material_type_id', $pid = 'material_type_pid', $son = 'children')
    {
        $tree = array();
        $tmpMap = array();
        //修复父类设置islink=0，但是子类仍然显示的bug @感谢linshaoneng提供代码
        foreach( $items as $item ){
            if( $item['material_type_pid']==0 ){
                $father_ids[] = $item['material_type_id'];
            }
        }
        //----
        foreach ($items as $item) {
        ...
        foreach ($items as $item) {
            //修复父类设置islink=0，但是子类仍然显示的bug by shaoneng @感谢linshaoneng提供代码
            if( $item['material_type_pid']<>0 && !in_array( $item['material_type_pid'], $father_ids )){
                continue;
            }
            //----
            if (isset($tmpMap[$item[$pid]])) {
    ...
    public function check_login(){
        ...
        if ($uid) {
            ...
            if ($user) {
                if ($auth ==  password($uid.$user['user'].$salt)) {
                    $flag = true;
                    $this->USER = $user;
                }
            }
