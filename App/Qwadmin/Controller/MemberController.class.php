...
class MemberController extends ComController
{
    public function index()
    {
        ...
        $count = $user->field("{$prefix}member.*,{$prefix}auth_group.id as gid,{$prefix}auth_group.title,{$prefix}auth_group.area_id")
            ->order($order)
        ...
        $list = $user->field("{$prefix}member.*,{$prefix}auth_group.id as gid,{$prefix}auth_group.title,{$prefix}auth_group.area_id")
            ->order($order)
            ->join("{$prefix}auth_group_access ON {$prefix}member.uid = {$prefix}auth_group_access.uid")
            ->join("{$prefix}auth_group ON {$prefix}auth_group.id = {$prefix}auth_group_access.group_id")
            ->where($where)
        ...
        $area_list = D('auth_area')->get_area_kv();
        foreach ($list as $key=>$val){
            foreach ($area_list as $a=>$b){
                if($val['area_id']==$a){
                    $list[$key]['area_name'] = $b;
                }
            }
        }                
        $this->assign('list', $list);
        $this->assign('page', $page);
        //$group = M('auth_group')->field('id,title,area_id')->select();
        //dump($list);exit;
    ...
    public function edit()
    {
        ...
        $usergroup = M('auth_group')->field('id,title,area_id')->select();
        $usergroup = D('auth_area')->getAreaName($usergroup);
    ...
    public function update($ajax = '')
    {
        ...
        $data['head'] = $head ? $head : 'http://'.$_SERVER['HTTP_HOST'].'/Public/attached/2017/05/09/591131b9c971e.png';
