...
class Tree {
	...
	*/
	public function get_parent($myid){
		...
		$pid = $this->arr[$myid]['material_type_pid'];
		$pid = $this->arr[$pid]['material_type_pid'];
		if(is_array($this->arr)){
			foreach($this->arr as $id => $a){
				if($a['material_type_pid'] == $pid) $newarr[$id] = $a;
			}
		}
		return $newarr;
	}
	...
	*/
	public function get_child($myid){
		...
		if(is_array($this->arr)){
			foreach($this->arr as $id=>$a){
                if($a['material_type_pid'] == $myid) $newarr[$id] = $a;
			}			
		}
	...
	*/
	public function get_tree($myid, $str, $sid = 0, $adds = '', $str_group = ''){
		...
		if(is_array($child)){
		    $total = count($child);
			foreach($child as $material_type_id=>$value){
				$j=$k='';
				if($number==$total){
					$j .= $this->icon[2];
				}else{
				...
				$spacer = $adds ? $adds.$j : '';
				$selected = $value['material_type_id']==$sid ? 'selected' : '';
				@extract($value);			
				$pid == 0 && $str_group ? eval("\$nstr = \"$str_group\";") : eval("\$nstr = \"$str\";");
				$this->ret .= $nstr;
				$nbsp = $this->nbsp;
				$this->get_tree($material_type_id, $str, $sid, $adds.$k.$nbsp,$str_group);
				$number++;
			}
		}
		return $this->ret;
